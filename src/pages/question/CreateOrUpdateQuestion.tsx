...
import {
    ...
    Resource,
    CategoryAttributes,
    UpdateQuestionParams, CreateQuestionParams, IdentityOptional
} from "@/types/apis";
...
const CreateOrUpdateQuestion = () => {
    ...
    useEffect(() => {
        ...
        if (isEditMode) {
            ...
            Promise.all([fetchQuestionData(), fetchData()]).catch((err) => {
    ...
    const handleQuestionTypeChange = useCallback((event: { target: { value: any; }; }) => {
        ...
        const newState = {
            ...
            type,
            hasQuillEditor: type !== QuestionType.TrueFalse && type !== QuestionType.FillIn,
        };
        ...
        switch (type) {
            ...
            case QuestionType.FillIn:
                ...
                newState.text = 'The capital of France is ${1} and the capital of Spain is ${2}.';
        ...
        try {
            ...
            if (isEditMode) {
                const questionPayload: UpdateQuestionParams = {
                    ...
                    passageId: questionData.passageId ? Number(questionData.passageId) : undefined,
                }
                ...
                dispatch(setMessageWithTimeout({ message: 'Question updated successfully', isError: false }));
            } else {
    ...
    return (
        <PageContainer title={isEditMode ? 'Edit Question' : 'Create Question'}
                       description={'This is the page to' + (isEditMode ? ' edit' : ' create') + ' a question'}>
                                    ...
                                    <Box sx={{ mt: 3 }}>
                                        <QuestionExplanation
                                            ...
                                            setExplanation={(value) => {
                                                handleQuestionDataChange({ ...questionData, explanation: { id: questionData.explanation?.id, content: value } })
                                            }}
                                        />
                                    </Box>
                                )}
