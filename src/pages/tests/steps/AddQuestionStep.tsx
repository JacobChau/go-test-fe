import {AssessmentQuestionAttributes, Identity, QuestionAttributes} from "@/types/apis";
...
import {QuestionColumns, QuestionSearchColumn} from "@/pages/question/components";
...
interface AddQuestionStepProps {
    ...
    setSelectedQuestions: React.Dispatch<React.SetStateAction<Map<number, AssessmentQuestionAttributes & Identity>>>;
}

const AddQuestionStep: FC<AddQuestionStepProps> = ({ selectedQuestions, setSelectedQuestions }) => {
    const {
        items: questions,
        searchTerm,
        filters,
    ...
    const handleSelectQuestion = (id: number, event: React.ChangeEvent<HTMLInputElement>, question: QuestionAttributes & Identity) => {
    ...
    const isQuestionSelected = (id: number) => selectedQuestions.has(id);
    ...
    return (
        ...
        <PageContainer title={"Add Questions to Test"} description={"Add questions to test"}>
                ...
                <BlankCard>
                    ...
                    <Grid container sx={{ marginBottom: 2 }}>
                        ...
                        <Grid item xs={12} sx={{ marginBottom: 1 }}>
                            <SearchComponent searchTerm={searchTerm} onSearchChange={handleSearchChange} searchColumn={QuestionSearchColumn} searchCriteria={searchCriteria} onSearchTermChange={handleSearchTermChange} />
                        </Grid>
                    </Grid>
                                    ...
                                    <TableRow key={question.id}>
                                        <TableCell padding="checkbox" sx={{ p: '0.5rem' }}>
                                            <Checkbox
                                                ...
                                                onChange={(event) => handleSelectQuestion(question.id, event, question)}
                                                inputProps={{ 'aria-label': 'select question' }}
                                            />
                                        </TableCell>
                                        {QuestionColumns.map((column, index) => (
                                            // @ts-ignore
                                            <TableCell key={index} sx={column.sx}>{column.render ? column.render(question) : question[column.key]}</TableCell>
                                        ))}
                                    </TableRow>
                                ))}
                            </TableBody>
