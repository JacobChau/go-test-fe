...
import {PublishAssessmentFormValues} from "@/pages/tests/CreateOrUpdateTest.tsx";

const PublishTestSchema = Yup.object().shape({
    validFrom: Yup.date()
        .min(dayjs().toDate(), "Start date and time must be after current date and time")
        .required('Start date and time is required'),
    validTo: Yup.date()
        .min(Yup.ref('validFrom'), "End date and time must be after start date and time")
...
const PublishTest: React.FC<PublishTestProps> = ({ onSubmit, formikRef, values }) => {
    return (
                                ...
                                <LocalizationProvider dateAdapter={AdapterDayjs}>
                                    <Grid item xs={12}>
                                        <DateTimePicker
                                            ...
                                            onChange={(value: Dayjs | null) => setFieldValue('validFrom', value)}
                                            slotProps={{
                                                textField: {
                                    ...
                                    <Grid item xs={12}>
                                        <DateTimePicker
                                            ...
                                            onChange={(value: Dayjs | null) => setFieldValue('validTo', value)}
                                            slotProps={{
                                                textField: {
