...
import {Formik, Form, Field, FormikHelpers, FormikErrors, FormikProps} from 'formik';
...
import {CreateAssessmentFormValues} from "@/pages/tests/CreateOrUpdateTest.tsx";
...
const CreateTestForm: React.FC<CreateTestFormProps> = ({subjects, values, onSubmit, formikRef}) => {
    const initialValues: CreateAssessmentFormValues = values;

    const handleFormSubmit = (values: CreateAssessmentFormValues, actions: FormikHelpers<CreateAssessmentFormValues>) => {
        onSubmit(values);
        actions.setSubmitting(false);
    };

    const handleNumericFieldChange = async (fieldName: string, e: { target: any; }, setFieldValue: { (field: string, value: any, shouldValidate?: boolean | undefined): Promise<void | FormikErrors<CreateAssessmentFormValues>>; (field: string, value: any, shouldValidate?: boolean | undefined): Promise<void | FormikErrors<CreateAssessmentFormValues>>; (arg0: any, arg1: string | number): void; }) => {
        let value = e.target.value === '' ? '' : Math.max(0, Math.min(1000, Number(e.target.value)));  // Use empty string instead of null
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12} sm={6}>
                                    ...
                                    >
                                        ...
                                        >
                                            {subjects.map((subject: Resource<SubjectAttributes>) => (
                                ...
                                <Grid item xs={12} sm={6}>
                                    <Field
                                        ...
                                        required
                                        variant="outlined"
                                        onChange={(e: { target: { value: string; }; }) => handleNumericFieldChange('totalMarks', e, setFieldValue)}
                                ...
                                <Grid item xs={12} sm={6}>
                                    <Field
                                        ...
                                        variant="outlined"
                                        type="number"
                                        onChange={(e: { target: any; }) => handleNumericFieldChange('passMarks', e, setFieldValue)}
                                    ...
                                    />
                                        ...
                                        <Grid item>
                                            <label htmlFor="duration">
                                                <Tooltip
                                                    title="Setting duration to 0 will indicate that the test has no time limit.">
                                    ...
                                    <Grid container alignItems="center" spacing={1}>
                                        ...
                                        <Grid item>
                                            <label htmlFor="maxAttempts">
                                                <Tooltip
                                                    title="Setting max attempts to 0 will indicate that the test has no attempt limit.">
                                                    <IconButton size="small">
                                                        <InfoIcon fontSize="inherit"/>
                                                    </IconButton>
                                                </Tooltip>
